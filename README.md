...
4. Run `npm run dev` from the repository root. This starts both the API and web app; do not start only Vite when using account creation, wallets, follows, community, or PPV.
...
Discovery and monetization: `GET /api/streams`, `GET /api/users/me/following`, `POST|DELETE /api/creators/:creatorId/follow`, `POST /api/streams/:id/access`
Community: `GET /api/community`, `POST /api/community/posts`, `GET /api/community/audience/export`
...
Live streams request a recorded Mux asset through `new_asset_settings`. Set `VOD_RETENTION_HOURS=48` to expose each recording as a 48-hour replay window. The long-running API deletes expired Mux assets hourly; serverless deployments must invoke the same cleanup routine from their platform scheduler.
...
When Supabase is configured, enable **Confirm email** under Authentication settings and add the deployed web URL to the allowed redirect URLs. Set `VITE_SUPABASE_REDIRECT_URL` to that URL. Signup then sends a confirmation email and does not authenticate the user until the link is completed; Supabase rejects unconfirmed login attempts.
